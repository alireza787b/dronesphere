...
class GotoCommand(BaseCommand):
    ...
    async def run(self, backend: AbstractBackend, **params) -> CommandResult:
        ...
        approach_speed = params.get("approach_speed", 0.5)  # m/s - speed when close to target
        
        # Determine positioning mode description
        mode_desc = "relative" if relative else "absolute"
        
        logger.info("goto_command_started", 
                   command_id=id(self),
        ...
        try:
            ...
            if not initial_telemetry or not initial_telemetry.position or initial_telemetry.position.north is None:
                return CommandResult(
                    success=False,
            ...
            if relative:
                ...
                logger.info("goto_relative_positioning",
                ...
                logger.info("goto_absolute_positioning",
                           target_north=target_north,
                           target_east=target_east,
                           target_down=target_down,
                           drone_id=backend.drone_id)
            ...
            logger.info("goto_target_calculated",
                       positioning_mode=mode_desc,
                       current_position={"north": current_pos.north, "east": current_pos.east, "down": current_pos.down},
                       target_position={"north": target_north, "east": target_east, "down": target_down},
            ...
            logger.info("executing_goto_offboard", 
                       drone_id=backend.drone_id, 
                       mode=mode_desc,
            ...
            if success:
                logger.info("goto_command_completed",
                           command_id=id(self),
                           positioning_mode=mode_desc,
                           target_north=target_north,
                           target_east=target_east,
            ...
            logger.info("goto_command_cancelled", 
                       command_id=id(self),
                       positioning_mode=mode_desc,
                       drone_id=backend.drone_id)
            return CommandResult(
            ...
            logger.error("goto_command_failed",
                        command_id=id(self), 
                        positioning_mode=mode_desc,
                        error=str(e),
                        drone_id=backend.drone_id)
    ...
    async def _wait_for_position_reached_with_verification(
        ...
        logger.info("waiting_for_position_with_verification", 
        ...
        async def check_position_reached():
            ...
            for check_num in range(max_checks):
                try:
                    ...
                    if (current_pos.north is None or 
                        current_pos.east is None or 
                        current_pos.down is None):
                        if check_num % 4 == 0:  # Log every 2 seconds
                            logger.warning("waiting_for_local_coordinates", 
                                         drone_id=backend.drone_id)
                        await asyncio.sleep(0.5)
                        continue
                    ...
                    if initial_position and initial_position.north is not None:
                        ...
                        max_distance_from_initial = max(max_distance_from_initial, distance_from_initial)
                    ...
                    if check_num % 8 == 0:  # Every 4 seconds (8 * 0.5s)
                        logger.info("goto_progress_detailed",
                                   current_north=current_pos.north,
                                   current_east=current_pos.east,
                                   current_down=current_pos.down,
                                   target_north=target_position.north,
                    ...
                    if distance_to_target <= tolerance:
                        logger.info("goto_position_reached",
                    ...
                    time_since_movement = asyncio.get_event_loop().time() - last_significant_movement_time
                    if time_since_movement > 10.0 and not self._movement_detected:
                        logger.warning("goto_no_movement_detected",
                    ...
                    if state in [DroneState.DISCONNECTED, DroneState.EMERGENCY]:
                        logger.error("goto_failed_bad_state", 
                                   state=state,
                                   drone_id=backend.drone_id)
                        self._final_distance = best_distance
                        return False
                    ...
                    logger.warning("goto_check_error", 
                                 error=str(e),
            ...
            # Timeout reached
            logger.warning("goto_timeout_reached", 
                          timeout=timeout,
