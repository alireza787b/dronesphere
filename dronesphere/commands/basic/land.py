...
class LandCommand(BaseCommand):
    ...
    async def run(self, backend: AbstractBackend, **params) -> CommandResult:
        ...
        try:
            ...
            logger.info(
                "land_command_cancelled", command_id=id(self), drone_id=backend.drone_id
            )
            return CommandResult(
                success=False, message="Land command was cancelled", error="Cancelled"
            )
            ...
            return CommandResult(
                success=False, message=f"Landing failed: {str(e)}", error=str(e)
            )
    ...
    async def _wait_for_landing_completion(
        self, backend: AbstractBackend, timeout: float
    ) -> bool:
        ...
        async def check_landed():
            ...
            for check_num in range(max_checks):
                try:
                    ...
                    if state == DroneState.DISCONNECTED:
                        logger.warning(
                            "landing_disconnected", drone_id=backend.drone_id
                        )
                        return False
                    ...
                    logger.warning(
                        "landing_check_failed", error=str(e), drone_id=backend.drone_id
                    )

                await asyncio.sleep(0.5)

            logger.warning(
                "landing_timeout", timeout=timeout, drone_id=backend.drone_id
            )
            return False
