...
class RtlCommand(BaseCommand):
    ...
    async def run(self, backend: AbstractBackend, **params) -> CommandResult:
        ...
        wait_for_landing = params.get(
            "wait_for_landing", True
        )  # Wait for complete RTL sequence
        ...
        try:
            ...
            success = await self._wait_for_rtl_completion(
                backend, timeout, wait_for_landing
            )
            ...
            if success:
                ...
                return CommandResult(
                    ...
                    error="RTL timeout",
                    duration=duration,
                    data={"rtl_duration": duration, "timeout": timeout},
                )

        except asyncio.CancelledError:
            logger.info(
                "rtl_command_cancelled", command_id=id(self), drone_id=backend.drone_id
            )
            return CommandResult(
                success=False, message="RTL command was cancelled", error="Cancelled"
            )
            ...
            return CommandResult(
                success=False, message=f"RTL failed: {str(e)}", error=str(e)
            )
        ...
        async def check_rtl_completed():
            ...
            for check_num in range(max_checks):
                try:
                    ...
                    logger.warning(
                        "rtl_check_failed", error=str(e), drone_id=backend.drone_id
                    )
