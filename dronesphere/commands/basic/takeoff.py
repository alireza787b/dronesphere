...
class TakeoffCommand(BaseCommand):
    ...
    async def run(self, backend: AbstractBackend, **params) -> CommandResult:
        ...
        altitude_tolerance = params.get("altitude_tolerance", 0.6)  # Accept 60% of target
        timeout = params.get("timeout", 20)  # Reduced timeout
        
        logger.info("takeoff_command_started", 
                   command_id=id(self),
                   altitude=altitude,
        ...
        try:
            ...
            # Execute takeoff
            logger.info("executing_takeoff", drone_id=backend.drone_id, altitude=altitude)
            ...
            if success:
                logger.info("takeoff_command_completed",
                           command_id=id(self),
                ...
                current_alt = current_telemetry.position.altitude_relative if current_telemetry.position else 0
                
                if current_alt > 0.5:  # Got some altitude
                    logger.warning("takeoff_partial_success",
                                 target_altitude=altitude,
            ...
            logger.info("takeoff_command_cancelled", 
                       command_id=id(self),
                       drone_id=backend.drone_id)
            return CommandResult(
                success=False,
            ...
            logger.error("takeoff_command_failed",
                        command_id=id(self), 
                        error=str(e),
                        drone_id=backend.drone_id)
            return CommandResult(
    ...
    async def _wait_for_takeoff_completion(
        ...
        logger.info("waiting_for_altitude", 
            ...
            for check_num in range(max_checks):
                try:
                    ...
                    if check_num % 4 == 0:  # Every 2 seconds (4 * 0.5s)
                        logger.info("takeoff_progress",
                                   current_altitude=current_altitude,
                                   target_minimum=target_min,
                                   target_full=target_altitude,
                                   progress_percent=int((current_altitude/target_altitude)*100),
                                   drone_id=backend.drone_id)
                    
                    # Success conditions
                    if current_altitude >= target_min:
                        logger.info("takeoff_altitude_achieved",
                    ...
                    if (check_num > 20 and  # After 10 seconds
                        state == DroneState.FLYING and 
                        current_altitude > 1.0):  # At least 1m
                        logger.info("takeoff_flying_assumed_success",
                    ...
                    if state in [DroneState.DISCONNECTED, DroneState.EMERGENCY]:
                        logger.error("takeoff_failed_bad_state", 
                                   state=state,
                                   drone_id=backend.drone_id)
                        return False, best_altitude
                        
                except Exception as e:
                    logger.warning("takeoff_check_error", 
                                 error=str(e),
                                 drone_id=backend.drone_id)
                    
                await asyncio.sleep(0.5)
                
            logger.warning("takeoff_timeout_reached", 
                          timeout=timeout,
