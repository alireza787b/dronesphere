...
class CommandRegistry:
    ...
    def register_implementation(
        self, name: str, implementation: type[BaseCommand]
    ) -> None:
        """Register a command implementation."""
        self._implementations[name] = implementation
        logger.info(
            "command_implementation_registered",
            command=name,
            impl_class=implementation.__name__,
        )
        ...
        try:
            ...
            for command_data in commands:
                try:
                    ...
                    logger.error(
                        "command_validation_failed", file=str(yaml_file), error=str(e)
                    )

        except Exception as e:
            logger.error("command_file_load_failed", file=str(yaml_file), error=str(e))

    logger.info(
        "command_library_loaded",
