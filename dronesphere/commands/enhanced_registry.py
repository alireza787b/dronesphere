...
from ..core.errors import CommandValidationError
...
class EnhancedCommandRegistry:
    ...
    def _load_yaml_specs(self) -> None:
        ...
        yaml_files = list(command_path.rglob("*.yaml")) + list(
            command_path.rglob("*.yml")
        )
        ...
        for yaml_file in yaml_files:
            try:
                ...
                logger.error("yaml_spec_load_failed", file=str(yaml_file), error=str(e))
    ...
    def _discover_implementations(self) -> None:
        ...
        for category in ["basic", "navigation", "utility", "advanced"]:
            try:
                ...
                logger.info(
                    "discovering_category", category=category, package=category_package
                )
                ...
                logger.warning(
                    "category_import_failed", category=category, error=str(e)
                )
    ...
    def list_commands(self) -> list[str]:
        ...
        return sorted(
            list(set(self._commands.keys()) & set(self._implementations.keys()))
        )
