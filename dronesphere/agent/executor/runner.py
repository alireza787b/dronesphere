...
class CommandRunner:
    ...
    def __init__(self, connection: DroneConnection):
        ...
        self.current_execution: CommandExecution | None = None
    ...
    async def enqueue_command(self, envelope: CommandEnvelope) -> str:
        ...
        command_request = CommandRequest(
            name=first_command.name, params=first_command.params or {}
        )
            ...
            try:
                ...
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        command_impl = registry.create_command(
                            command.name, command.params or {}
                        )
                        ...
                        result = await command_impl.run(
                            self.connection.backend, **command_impl.parameters
                        )
                ...
                if result and result.success:
                    ...
                    if is_critical:
                        ...
                        logger.warning(
                            "noncritical_command_failed_continuing",
                            command=command.name,
                            error=last_error,
                        )
                        continue
                ...
                logger.error(
                    "command_execution_error", command=command.name, error=str(e)
                )
    ...
    async def _execute_failsafe(self, failsafe_action: str) -> None:
        ...
        try:
            ...
            if failsafe_action == "land":
                ...
                result = await land_command.run(
                    self.connection.backend, **land_command.parameters
                )
                ...
                result = await rtl_command.run(
                    self.connection.backend, **rtl_command.parameters
                )
            ...
            logger.error(
                "failsafe_execution_failed", action=failsafe_action, error=str(e)
            )

    def get_current_execution(self) -> CommandExecution | None:
