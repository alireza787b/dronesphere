...
class AgentConfig(BaseModel):
    ...
    drone_connection_string: str = Field(
        default="udp://:14540", description="MAVLink connection string"
    )
    telemetry_update_interval: float = Field(
        default=0.25, description="Telemetry update interval in seconds"
    )
    command_timeout: int = Field(
        default=300, description="Default command timeout in seconds"
    )
...
class BackendConfig(BaseModel):
    ...
    default_backend: Literal["mavsdk", "pymavlink"] = Field(
        default="mavsdk", description="Default drone backend"
    )
    telemetry_backend: Literal["mavsdk", "pymavlink"] = Field(
        default="mavsdk", description="Telemetry backend"
    )
...
class PathConfig(BaseModel):
    ...
    shared_config_path: Path = Field(
        default=Path("./shared"), description="Shared configuration directory"
    )
    command_library_path: Path = Field(
        default=Path("./shared/commands"), description="Command library directory"
    )
