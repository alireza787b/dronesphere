...
class TelemetryCache:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "telemetry_cache_starting", interval=self.settings.telemetry_cache_interval
        )
    ...
    async def _cache_loop(self) -> None:
        ...
        while self._running:
            ...
            await asyncio.sleep(
                max(2.0, self.settings.telemetry_cache_interval)
    ...
    async def _update_cache(self) -> None:
        ...
        for drone in connected_drones:
            task = self._get_drone_telemetry(
                drone.id, drone.agent_host, drone.agent_port
            )
            tasks.append(task)
        ...
        if tasks:
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    logger.warning(
                        "telemetry_fetch_failed", drone_id=drone.id, error=str(result)
                    )
                elif result:
                    self.cache[drone.id] = {**result, "cached_at": time.time()}

    async def _get_drone_telemetry(
        self, drone_id: int, agent_host: str, agent_port: int
    ) -> dict[str, Any] | None:
        """Get telemetry from specific drone."""
        return await self.fleet_manager.client.get_agent_telemetry(
            agent_host, agent_port
        )

    def get_drone_telemetry(self, drone_id: int) -> dict[str, Any] | None:
    ...
    def get_cache_age(self, drone_id: int) -> float | None:
