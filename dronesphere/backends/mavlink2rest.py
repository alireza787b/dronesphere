...
from ..core.utils import run_with_timeout, safe_float, safe_int
...
class Mavlink2RestBackend(AbstractBackend):
    ...
    async def connect(self) -> None:
        ...
        try:
            logger.info("connecting_to_mavlink2rest", drone_id=self.drone_id, 
                       url=self.base_url)
            ...
            logger.error("mavlink2rest_connection_failed", drone_id=self.drone_id, error=str(e))
            await self._cleanup_connection()
            raise DroneConnectionError(f"Failed to connect to mavlink2rest {self.drone_id}: {e}")
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            logger.error("mavlink2rest_disconnect_failed", drone_id=self.drone_id, error=str(e))
    ...
    async def takeoff(self, altitude: float) -> None:
        ...
        try:
            ...
            logger.info("takeoff_completed_rest", drone_id=self.drone_id, altitude=altitude)
    ...
    async def goto_position(self, position: Position, yaw: Optional[float] = None, max_speed: float = 2.0) -> None:
        ...
        try:
            logger.info("goto_position_rest", drone_id=self.drone_id, 
            ...
            logger.error("goto_position_failed_rest", drone_id=self.drone_id, error=str(e))
    ...
    async def set_flight_mode(self, mode: FlightMode) -> None:
        ...
        try:
            ...
            logger.error("set_flight_mode_failed_rest", drone_id=self.drone_id, error=str(e))
    ...
    async def get_telemetry(self) -> Telemetry:
        ...
        try:
            ...
            logger.error("get_telemetry_failed_rest", drone_id=self.drone_id, error=str(e))
    ...
    async def _test_connection(self) -> None:
        ...
        try:
            ...
            logger.info("mavlink2rest_vehicles_discovered", vehicles=vehicles, drone_id=self.drone_id)
    ...
    async def _discover_target_system(self) -> None:
        ...
        try:
            ...
            if heartbeat:
                ...
                self._target_system = 1
                self._target_component = 1
                logger.info("target_system_discovered", 
            ...
            logger.warning("target_discovery_failed", error=str(e), drone_id=self.drone_id)
    ...
    async def _start_heartbeat_monitoring(self) -> None:
        ...
        try:
            ...
            logger.warning("heartbeat_monitoring_failed", error=str(e), drone_id=self.drone_id)
    ...
    async def _wait_for_systems_ready(self) -> None:
        ...
        for i in range(max_retries):
            try:
                ...
                if i == max_retries - 1:
                    logger.warning("systems_not_fully_ready", error=str(e), drone_id=self.drone_id)
                await asyncio.sleep(1.0)
    ...
    async def _send_command_long(self, command: int, param1: float = 0, param2: float = 0,
        ...
        try:
            url = urljoin(self.base_url, f"mavlink/vehicles/{self._target_system}/components/{self._target_component}/messages/COMMAND_LONG")
            ...
            try:
                ...
                if ack and ack.get("command") == command:
                    ...
                    if result == 0:  # MAV_RESULT_ACCEPTED
                        ...
                        raise BackendError(f"Command {command} rejected with result: {result}")
    ...
    async def _wait_for_altitude(self, target_altitude: float, timeout: float = 60.0) -> None:
        ...
        while time.time() - start_time < timeout:
            try:
                global_pos = await self._get_message("GLOBAL_POSITION_INT")
                if global_pos:
                    current_altitude = abs(safe_float(global_pos.get("relative_alt", 0)) / 1000.0)  # mm to m
                    if current_altitude >= target_altitude:
                        logger.info("altitude_reached_rest", 
    ...
    async def _send_position_target_local_ned(self) -> None:
        ...
        try:
            url = urljoin(self.base_url, f"mavlink/vehicles/{self._target_system}/components/{self._target_component}/messages/SET_POSITION_TARGET_LOCAL_NED")
            ...
            logger.warning("position_target_send_failed", error=str(e), drone_id=self.drone_id)
    ...
    async def _offboard_position_sender(self) -> None:
        ...
        while self._offboard_active and self._position_target:
            try:
                ...
                logger.warning("offboard_sender_error", error=str(e), drone_id=self.drone_id)
                await asyncio.sleep(0.5)
    
    async def _get_message(self, message_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            urls = [
                urljoin(self.base_url, f"mavlink/vehicles/{self._target_system}/components/{self._target_component}/messages/{message_type}"),
                urljoin(self.base_url, f"mavlink/vehicles/{self._target_system}/messages/{message_type}"),
                urljoin(self.base_url, f"vehicles/{self._target_system}/components/{self._target_component}/messages/{message_type}")
            ]
    ...
    def _build_position(self, data: Dict[str, Any]) -> Optional[Position]:
        ...
        if global_pos:
            ...
            position.altitude_msl = safe_float(global_pos.get("alt", 0)) / 1000  # mm to m
            position.altitude_relative = safe_float(global_pos.get("relative_alt", 0)) / 1000  # mm to m
    ...
    def _build_attitude(self, data: Dict[str, Any]) -> Optional[Attitude]:
    ...
    def _build_velocity(self, data: Dict[str, Any]) -> Optional[Velocity]:
        ...
        return velocity if any([velocity.north, velocity.east, velocity.ground_speed]) else None
    
    def _build_battery(self, data: Dict[str, Any]) -> Optional[Battery]:
    ...
    def _build_gps(self, data: Dict[str, Any]) -> Optional[GPS]:
        ...
        return GPS(
            ...
            hdop=safe_float(gps_msg.get("eph", 0)) / 100 if gps_msg.get("eph", 65535) != 65535 else None,
            vdop=safe_float(gps_msg.get("epv", 0)) / 100 if gps_msg.get("epv", 65535) != 65535 else None
        )
...
class Mavlink2RestTelemetryProvider(TelemetryProvider):
    ...
    def __init__(self, drone_id: int, connection_string: str):
        super().__init__(drone_id, connection_string)
        self._backend: Optional[Mavlink2RestBackend] = None
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.error("mavlink2rest_telemetry_connection_failed", drone_id=self.drone_id, error=str(e))
