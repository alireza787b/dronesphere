...
class PyMavlinkBackend(AbstractBackend):
    ...
    async def return_to_launch(self) -> None:
        ...
        try:
            ...
            # Send RTL command
            await self._send_command_long(
                mavlink.MAV_CMD_NAV_RETURN_TO_LAUNCH, timeout=120.0
            )
    ...
    async def goto_position(
        ...
        try:
            ...
            await self._set_position_target_local_ned(
                position.north, position.east, position.down, yaw or 0.0, max_speed
            )
    ...
    async def _create_connection(
        ...
        try:
            ...
            logger.error(
                "connection_creation_failed", error=str(e), drone_id=self.drone_id
            )
            raise
        ...
        while time.time() - start_time < timeout:
            ...
            msg = await loop.run_in_executor(
                ...
                lambda: self.connection.recv_match(
                    type="HEARTBEAT", blocking=False, timeout=0.1
                ),
            )
        ...
        while self._connected and self.connection:
            try:
                ...
                await loop.run_in_executor(
                    None, lambda: self.connection.write(heartbeat_msg)
                )
                ...
                logger.error(
                    "heartbeat_sender_error", error=str(e), drone_id=self.drone_id
                )
                break
    ...
    async def _cleanup_connection(self) -> None:
        ...
        if self.connection:
            try:
                ...
                logger.error(
                    "connection_close_error", error=str(e), drone_id=self.drone_id
                )
            finally:
        ...
        while time.time() - start_time < timeout:
            ...
            if ack_msg and ack_msg.command == command:
                if ack_msg.result == mavlink.MAV_RESULT_ACCEPTED:
                    logger.debug(
                        "command_accepted", command=command, drone_id=self.drone_id
                    )
                    return
                else:
    ...
    async def _wait_for_altitude(
        ...
        while time.time() - start_time < timeout:
            ...
            msg = await loop.run_in_executor(
                None,
                lambda: self.connection.recv_match(
                    type="VFR_HUD", blocking=False, timeout=0.1
                ),
            )
    ...
    async def _set_position_target_local_ned(
        self, north: float, east: float, down: float, yaw: float, max_speed: float
    ) -> None:
    ...
    async def _get_position_telemetry(self) -> Position | None:
        ...
        try:
            ...
            logger.debug(
                "position_telemetry_failed", error=str(e), drone_id=self.drone_id
            )

        return None

    async def _get_attitude_telemetry(self) -> Attitude | None:
        ...
        try:
            ...
            logger.debug(
                "attitude_telemetry_failed", error=str(e), drone_id=self.drone_id
            )

        return None

    async def _get_velocity_telemetry(self) -> Velocity | None:
        ...
        try:
            ...
            logger.debug(
                "velocity_telemetry_failed", error=str(e), drone_id=self.drone_id
            )

        return None

    async def _get_battery_telemetry(self) -> Battery | None:
        ...
        try:
            ...
            msg = await loop.run_in_executor(
                None,
                lambda: self.connection.recv_match(
                    type="SYS_STATUS", blocking=False, timeout=0.1
                ),
            )
            ...
            logger.debug(
                "battery_telemetry_failed", error=str(e), drone_id=self.drone_id
            )

        return None

    async def _get_gps_telemetry(self) -> GPS | None:
    ...
    async def _get_local_ned_from_mavlink2rest(self) -> dict[str, float] | None:
        ...
        try:
            ...
            logger.debug(
                "mavlink2rest_ned_failed", error=str(e), drone_id=self.drone_id
            )
...
class PyMavlinkTelemetryProvider(TelemetryProvider):
    ...
    def __init__(self, drone_id: int, connection_string: str):
        super().__init__(drone_id, connection_string)
        self._backend: PyMavlinkBackend | None = None
