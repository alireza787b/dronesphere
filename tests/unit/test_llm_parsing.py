...
from server.services.llm.parsing import create_command_parser
        ...
        for i, test_input in enumerate(test_inputs):
            ...
            if expected_commands[i]:
                ...
                if expected_altitudes[i] is not None:
                    assert (
                        result["commands"][0]["parameters"]["altitude"]
                        == expected_altitudes[i]
                    )
                ...
                "commands": [
                    {
                        "name": "takeoff",
                        "category": "flight",
                        "parameters": {"altitude": 20, "speed": "normal"},
                        "confidence": 0.9,
...
async def test_integration_with_openrouter():
    ...
    # Test context
    context = ConversationContext(session_id="test-session", drone_id="test-drone")
    ...
    result = await provider.extract_commands(
        "Please take off to 15 meters", context, commands
    )
