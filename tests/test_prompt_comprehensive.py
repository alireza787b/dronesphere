...
async def test_comprehensive():
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        try:
            result = await provider.extract_commands(test["input"], context, commands)
            ...
            if "expected_commands" in test:
                if len(result.commands) != test["expected_commands"]:
                    print(
                        f"❌ Expected {test['expected_commands']} commands, got {len(result.commands)}"
                    )
                    checks_passed = False
                else:
                    print("✅ Command count correct")

            # Check command name
            if "expected_name" in test and result.commands:
                if result.commands[0]["name"] != test["expected_name"]:
                    print(
                        f"❌ Expected command '{test['expected_name']}', got '{result.commands[0]['name']}'"
                    )
                    checks_passed = False
                else:
                    print("✅ Command name correct")
            ...
            if "expected_params" in test and result.commands:
                ...
                for key, value in test["expected_params"].items():
                    if key not in params or params[key] != value:
                        print(
                            f"❌ Parameter {key}: expected {value}, got {params.get(key)}"
                        )
                        checks_passed = False
                    else:
            ...
            if "expected_sequence" in test:
                ...
                if actual_sequence != test["expected_sequence"]:
                    print(
                        f"❌ Expected sequence {test['expected_sequence']}, got {actual_sequence}"
                    )
                    checks_passed = False
                else:
                    print("✅ Command sequence correct")

            # Check clarification
            if "expected_clarification" in test:
                if result.requires_clarification != test["expected_clarification"]:
                    print(
                        f"❌ Expected clarification={test['expected_clarification']}, got {result.requires_clarification}"
                    )
                    checks_passed = False
                else:
                    print("✅ Clarification requirement correct")

            # Check language
            if "expected_language" in test:
                if result.detected_language != test["expected_language"]:
                    print(
                        f"❌ Expected language '{test['expected_language']}', got '{result.detected_language}'"
                    )
                    checks_passed = False
                else:
