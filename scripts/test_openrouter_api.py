...
SYSTEM_MESSAGE = "You are a helpful and concise AI assistant. Provide direct and factual answers."
...
RETRY_DELAY_SECONDS = 5 # Delay in seconds between retry attempts to avoid hammering the API.
...
def run_llm_test():
    ...
    if not api_key:
        ...
        print("  Example for Linux/macOS: export OPENROUTER_API_KEY=\"sk-or-v1-YOUR_KEY_HERE\"")
        print("  Example for Windows (CMD): set OPENROUTER_API_KEY=\"sk-or-v1-YOUR_KEY_HERE\"")
        print("  Example for Windows (PowerShell): $env:OPENROUTER_API_KEY=\"sk-or-v1-YOUR_KEY_HERE\"")
    ...
    print("\n" + "="*80)
    print(f"--- OpenRouter LLM Test Tool v1.0 ---".center(80))
    ...
    for attempt in range(1, NUM_RETRIES + 1):
        print(f"Attempt {attempt}/{NUM_RETRIES}: Sending request to '{MODEL_TO_TEST}'...")
        ...
        try:
            ...
            if completion.choices:
                ...
                print(f"Finish Reason: {finish_reason} (Indicates why the model stopped generating)")
                
                if assistant_response and assistant_response.strip():
                    print("\n" + "="*20 + " Assistant's Reply " + "="*20)
                    print(assistant_response.strip())
                    print("="*61 + "\n")
                    print("--- Status: TEST SUCCESSFUL! LLM provided a meaningful response. ---")
                    successful_response_received = True
                else:
                    print("\n--- WARNING: Empty or Whitespace-Only Response Content ---")
                    print("The model responded, but the content was empty or just whitespace.")
                    print(f"Raw content received: '{assistant_response}'")
                    print(f"Finish Reason: {finish_reason}")
                    print("This often indicates high load on free models, or a very short `max_tokens`.")
                    
                # Always print token usage if available for transparency
                if completion.usage:
                    print("\n--- Token Usage Metrics ---")
                    print(f"  Prompt Tokens (Input): {completion.usage.prompt_tokens}")
                    print(f"  Completion Tokens (Output): {completion.usage.completion_tokens}")
                    print(f"  Total Tokens: {completion.usage.total_tokens}")
                    print("Note: For free models, these might be reported as 0/0 or actual usage.")
                print("-"*(60 + len(" LLM Response Analysis ")) + "\n")

            else:
                print("\n--- WARNING: No Message Choices Found in Response ---")
                print("The API call was successful, but the model did not return any 'choices' in its response.")
                print("This is unusual and suggests an unexpected API response structure or an issue with the model.")
                print("Dumping full completion object for developer debugging:")
                print(json.dumps(completion.model_dump(), indent=2)) # Use model_dump() for pydantic object
                print("-"*(60 + len(" LLM Response Analysis ")) + "\n")
            ...
            print("Please regenerate your OpenRouter API key and update your environment variable.")
            print("!"*80 + "\n")
            ...
            print("Consider: waiting longer, reducing NUM_RETRIES/RETRY_DELAY_SECONDS, or upgrading your OpenRouter plan.")
            print("!"*80 + "\n")
            ...
            if e.status_code == 400:
                print("- Bad Request: Check your prompt format or API parameters against OpenRouter documentation.")
            elif e.status_code == 404:
                print("- Not Found: The model name might be incorrect or unavailable: '{MODEL_TO_TEST}'.")
            elif e.status_code >= 500:
                print("- Server Error: OpenRouter or the model provider might be experiencing temporary issues.")
            print("-------------------------------------------\n")
            if attempt < NUM_RETRIES:
                print(f"Retrying in {RETRY_DELAY_SECONDS} seconds...")
                time.sleep(RETRY_DELAY_SECONDS)
            ...
            print("Given your location (Iran), network connectivity to external APIs can sometimes be challenging.")
            print("------------------------\n")
            if attempt < NUM_RETRIES:
                print(f"Retrying in {RETRY_DELAY_SECONDS} seconds...")
                time.sleep(RETRY_DELAY_SECONDS)
    ...
    if not successful_response_received:
        print("\n" + "="*80)
        print("!!! FINAL STATUS: ALL ATTEMPTS FAILED TO GET A MEANINGFUL RESPONSE !!!".center(80))
        print("\nRecommendations:")
        ...
        print("  - Consider using a different LLM model from OpenRouter (`openrouter.ai/models`).")
        print("  - The free tier might be under very heavy load; try again later.")
        print("  - Review your OpenRouter account usage and limits.")
        print("  - Investigate network stability, especially if you are in Iran, as connections to external services can be interrupted.")
        print("="*80 + "\n")
