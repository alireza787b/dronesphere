...
async def test_ned_methods():
    ...
    telemetry_methods = [
        method for method in dir(drone.telemetry) if not method.startswith("_")
    ]
    ned_methods = [
        method
        for method in telemetry_methods
        if "ned" in method.lower() or "position" in method.lower()
    ]
    ...
    print(
        f"   - telemetry.position(): {'✅ EXISTS' if hasattr(drone.telemetry, 'position') else '❌ MISSING'}"
    )
    print(
        f"   - telemetry.position_ned(): {'✅ EXISTS' if hasattr(drone.telemetry, 'position_ned') else '❌ MISSING'}"
    )
    print(
        f"   - telemetry.position_velocity_ned(): {'✅ EXISTS' if hasattr(drone.telemetry, 'position_velocity_ned') else '❌ MISSING'}"
    )
    ...
    async for health in drone.telemetry.health():
        if hasattr(health, "is_local_position_ok") and health.is_local_position_ok:
            ...
            print(
                f"⏳ Local position not ready (is_local_position_ok: {health.is_local_position_ok})"
            )
        else:
    ...
    try:
        async for pos_ned in drone.telemetry.position_ned():
            print(
                f"✅ Got position_ned: North={pos_ned.north_m}, East={pos_ned.east_m}, Down={pos_ned.down_m}"
            )
            break
    ...
    try:
        async for odom in drone.telemetry.position_velocity_ned():
            pos = odom.position
            vel = odom.velocity
            print("✅ Got position_velocity_ned:")
            print(
                f"   Position: North={pos.north_m:.3f}m, East={pos.east_m:.3f}m, Down={pos.down_m:.3f}m"
            )
            print(
                f"   Velocity: North={vel.north_m_s:.3f}m/s, East={vel.east_m_s:.3f}m/s, Down={vel.down_m_s:.3f}m/s"
            )
            break
    except AttributeError as e:
    ...
    try:
        async for pos in drone.telemetry.position():
            ...
            print(
                f"   Alt MSL: {pos.absolute_altitude_m:.3f}m, Alt Rel: {pos.relative_altitude_m:.3f}m"
            )
            break
    except Exception as e:
