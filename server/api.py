...
DRONE_REGISTRY = {1: "127.0.0.1:8001"}  # drone_id: ip:port
...
async def route_commands(request: dict) -> Dict[str, Any]:
    ...
    try:
        ...
        print(
            f"🎯 Routing {len(request.get('commands', []))} commands to drone {target_drone} at {drone_endpoint}"
        )
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                raise HTTPException(
                    status_code=504, detail=f"Timeout waiting for drone {target_drone} response"
                )
...
async def fleet_health() -> Dict[str, Any]:
    ...
    """
    health_status = {"timestamp": time.time(), "total_drones": len(DRONE_REGISTRY), "drones": {}}
