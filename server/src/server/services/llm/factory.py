...
class LLMProviderFactory:
    ...
    @classmethod
    def create_provider(
        cls, config: BaseLLMConfig, fallback_config: Optional[BaseLLMConfig] = None
    ) -> BaseLLMProvider:
        """
    ...
    def create_from_settings(
        cls, settings: Any, fallback_provider: Optional[str] = None
    ) -> BaseLLMProvider:
        """
    ...
    def _config_from_settings(cls, settings: Any, provider_name: str) -> BaseLLMConfig:
        ...
        if provider_lower == "openrouter":
            return OpenRouterConfig(
                ...
                or getattr(settings, "openai_api_key", "dummy-key-for-free-model"),
                base_url=getattr(
...
async def get_llm_provider(settings: Any) -> BaseLLMProvider:
    ...
    return await LLMProviderFactory.create_with_health_check(
        ...
        (
            LLMProviderFactory._config_from_settings(
                settings, "ollama"
            )  # Fallback to Ollama
            if hasattr(settings, "llm_fallback_provider")
            and settings.llm_fallback_provider
            else None
        ),
    )
