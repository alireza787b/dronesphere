...
from .base import CommandExtractionResult
...
class RobustCommandParser:
        ...
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                ...
                if normalized_key == k and k != k.lower():
                    ...
                    if lowercase_key in ["commands", "response_text", "requires_clarification",
                                        "clarification_questions", "detected_language", 
                                        "overall_confidence", "original_phrase"]:
    ...
    def _extract_fallback_data(self, text: str) -> Dict[str, Any]:
        ...
        if "takeoff" in text_lower or "take off" in text_lower:
            altitude_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:meter|metre|m)', text_lower)
        ...
        # Detect language
        detected_language = "fa" if any(ord(c) > 0x0600 and ord(c) < 0x06FF for c in text) else "en"
        ...
        return {
            ...
            "clarification_questions": ["Could you please rephrase your command?"] if len(commands) == 0 else [],
            "detected_language": detected_language,
