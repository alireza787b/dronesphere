...
class BasePromptTemplate(ABC):
    ...
    def render(self, variables: Dict[str, Any]) -> str:
        ...
        for section in sorted_sections:
            if not section.required and not self._should_include_section(section, variables):
                continue
            ...
            try:
                ...
                if section.required:
                    raise ValueError(f"Missing required variable for {section.layer}: {e}")
                logger.debug(f"Skipping optional section {section.layer} due to missing variable: {e}")
        
        return "\n\n".join(rendered_parts)
    
    def _should_include_section(self, section: PromptSection, variables: Dict[str, Any]) -> bool:
